...
$med_full_sql="SELECT medicines,medicine_quantity,day_count,medicine_intake,time_intervals FROM $tbl_name WHERE patient_id='$patient_id'";


 $med_full_result=mysql_query($med_full_sql);

 $med_sql="SELECT medicines,medicine_quantity,day_count,medicine_intake,time_intervals FROM $tbl_name WHERE patient_id='$patient_id'";


 $med_result=mysql_query($med_sql);
 
 $med_row=mysql_fetch_assoc($med_result);
 
 $messages = 'Medicines '. $med_row['medicines']. ', Medicine Quantity '. $med_row['medicine_quantity']. ' Day count '. $med_row['day_count']. ', Medicine Intake '. $med_row['medicine_intake']. ' Time Intervals ' .$med_row['time_intervals'];



 $sqlP="SELECT id, name, mobile_no FROM `patient` WHERE name='$patient_id'";
 $rowP=mysql_query($sqlP);
	
 
 while ($rowP = mysql_fetch_array($rowP)) {
	$name = $rowP['name']; 
	$id = $rowP['id']; 
...
<table>
  <tr>
    ...
    <th>TIME INTERVAL</th>
					...
					<?php
					while ($row = mysql_fetch_array($med_full_result))
{
...
<p class="ex1" style="text-align: left; color: #000">Doctor's signature:</p>
					...
					<h3 class="ex1" style="text-align: left; color: #000">DONT's OF TAKING MEDICATIONS:</h3>
						<p class="ex1" style="text-align: left; color: #000">1.Do not share your medication with someone else
								...
								<p class="ex1" style="text-align: left; color: #000">2.Do not use medication when it is past its expiry date
 </p>			
...
<div>						
						<input type="submit" value="Send Message" name="send" style="width: 150px; border-radius:10px; height:50px; text-decoration: none;">
						<br>
						<br>
						<div class="container-login100-form-btn">
