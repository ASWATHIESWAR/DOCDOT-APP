...
$pat_query="select name from patient where id=$pat_id";
$pat_query_run =mysql_query($pat_query);
$pat_row=mysql_fetch_assoc($pat_query_run);
?>
<select name='patient' onchange="getPatient(this.value)">
...
while ($select_query_array=mysql_fetch_array($select_query_run) )
{
   echo "<option value='".$select_query_array['name']."' >".htmlspecialchars($select_query_array["name"])."</option>";
}
echo "</select>";
?>
						<span class="focus-input100" data-placeholder=""></span>
...
<button type="submit" form="prescription_page.php" value="addother" name="addother" class="login100-form-btn">Add Other Medicine</button>             
					</div>
					<br>
					  <div class="container-login100-form-btn">
			 
<button type="submit" form="prescription_page.php"  name="create" value="create" class="login100-form-btn">Create Prescription</button>             
					</div>
          
          <br>
          <!-- <a href="prescription_page.php?id=<?php echo $_GET['patient_id'];?>"> </a> -->
...
function myFunction(val) {
	...
	// if(arr[4]) {
	// var newVal = arr[0] +'&'+ arr[1] +'&'+ arr[2] +'&'+ arr[3] + '&'+ arr[4] +"&med_id="+val;
	// } else {
	 var newVal = arr[0] +'&'+ arr[1] +'&'+ arr[2] +'&'+ arr[3] + "&med_id="+val;
	// }
	console.log(arr);
...
var loc = window.location;
	 ...
	 var arr = old.split("&");
	  if(arr[4]) {
	 var newVal = arr[0] +'&'+ arr[1] +'&'+ arr[2] +'&'+ arr[3] + '&'+ arr[4] +"&patient_id="+val;
	 } else {
	 var newVal = arr[0] +'&'+ arr[1] +'&'+ arr[2] +'&'+ arr[3] + "&patient_id="+val;
	 }
	 console.log(arr);
	 window.location = newVal;
}
